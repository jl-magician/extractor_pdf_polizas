...
def test_extract_all_fields(sample_ingestion_result, valid_extraction_data):
    ...
    with patch("anthropic.Anthropic") as MockClient:
        ...
        result = extract_policy(sample_ingestion_result)
...
def test_spanish_and_english(lang, page_text, expected_aseguradora, valid_extraction_data):
    ...
    with patch("anthropic.Anthropic") as MockClient:
        ...
        from policy_extractor.schemas.poliza import PolicyExtraction
        result = extract_policy(ingestion)
...
def test_raw_response_stored(sample_ingestion_result, valid_extraction_data):
    ...
    with patch("anthropic.Anthropic") as MockClient:
        ...
        # extract_policy should return a tuple: (PolicyExtraction, raw_response_dict)
        # OR store the raw response as an attribute on the result
        result = extract_policy(sample_ingestion_result)

    # The function should return either a tuple or the raw response should be accessible
    # Accept either: tuple return value or campos_adicionales containing raw_response key
    from policy_extractor.schemas.poliza import PolicyExtraction

    if isinstance(result, tuple):
        policy, raw = result
        assert isinstance(policy, PolicyExtraction)
        assert isinstance(raw, dict)
    else:
        # Raw response stored in campos_adicionales or as __raw_response attribute
        assert isinstance(result, PolicyExtraction)
        has_raw = (
