...
- [x] **QAL-01**: User can run Sonnet evaluation on an extraction via `--evaluate` CLI flag
...
- [x] **QAL-04**: Evaluation is opt-in only — never runs in the default extraction path
- [x] **QAL-05**: API upload endpoint accepts optional `evaluate=true` query parameter
